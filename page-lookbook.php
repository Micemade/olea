...
<div class="row">
	...
	<div id="primary" class="large-<?php echo ( $layout =='full_width' || $shop_full ) ? '12' : '8'; ?> medium-12 small-12 <?php echo $layout ? esc_attr( $layout ) : null; ?>" role="main">
		...
		?>
				...
				if( !empty($term_Objects) ) {
					...
					echo '<li class="all category-link"><a href="#" class="active"><div class="term">'. esc_html__('All','olea') .'</div></a></li>';
					...
					echo '<div class="lookbook-empty">';
						...
						echo '<h3>'. esc_html__("No lookbook categories containing lookbook items selected.","olea")  .'</h3>';
												
						if( current_user_can('editor') || current_user_can('administrator')  ) {
							
							echo '<p><strong>' . esc_html__("Please, do the following:","olea") . '</strong></p>';
							echo '<ul>';
								echo '<li>'. esc_html__("Make sure that some lookbook categories are created","olea") .'</li>';
								echo '<li>'. esc_html__("Lookbook items are assigned to categories","olea") .'</li>';
								echo '<li>'. esc_html__("In lookbook page template select lookbook categories for display","olea") .'</li>';
							echo '</ul>';
						}else{
							
							echo '<p><strong>' . esc_html__("Currently there are no lookbook items set. Please, come back soon.","olea") . '</strong></p>';
						}
					...
					<div id="post-<?php the_ID(); ?>" <?php post_class($classes); ?>>
						...
						<div class="item-data lookbook-data">
							...
							if( count( $productsIDs ) && !empty($productsIDs[0]) ) {
								...
								echo '<span class="lookbook-item-products">' . __('Products from this combination','olea') .':</span>';
								...
								echo '<span class="lookbook-item-products">' . __('Products from this combination not available','olea') .'.</span>';
