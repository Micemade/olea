...
?>
<div class="testimonials<?php echo (count($testimonial_arr) > 1) ? ' owl-carousel contentslides' : ''; ?> ">
...
foreach( $testimonial_arr as $testimonial ) {
	...
	$output .= '<div class="testimonial-item column item '. esc_attr($anim) .'" data-i="'. esc_attr($i) .'">';
		...
		if( $img ) {
			...
			$output .= '<div class="image '. esc_attr($image_style) .'">';
		...
		$no_img = !$img ? ' width: 100%; margin-top: 0; padding-top:0;' : '';
		$t_color = $text_color ? ('color:'. $text_color .';') : '';
		$inline_style = ( ( $t_color || $no_img ) ? ' style="' : '' ) . ( $t_color ? $t_color : '' ) . ( $no_img  ? $no_img  : '') . ( ( $t_color || $no_img ) ? '"' : '' );
		
		$output .= '<div class="content"'. $inline_style .'>';
