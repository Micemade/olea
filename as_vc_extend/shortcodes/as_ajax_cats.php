...
function as_ajax_cats_func( $atts, $content = null ) { 
	...
	$sticky_array = get_option( 'sticky_posts' );
		...
		if( $tax_terms && $tax_menu_style != 'none') {
			...
			echo $tax_menu_style == 'dropdown' ?'<div class="menu-toggler block-tax-toggler"><a href="#" title="'.esc_attr(__('Toggle categories','olea')).'"><span class="icon-menu"></span></a></div>' : null;
			?>
		...
		*/
		...
		<div id="posts-<?php echo esc_attr($block_id); ?>" class="content-block cb-2">
			...
			<div class="<?php echo !$hide_slider ? 'owl-carousel contentslides' : 'js-masonry';?> category-content <?php echo ' '. esc_attr($anim) ;?> <?php echo $data_anim == 'none' ? '' : esc_attr($data_anim); ?><?php echo ' '. esc_attr($block_style); ?>" id="ajax-cats-<?php echo esc_attr($block_id); ?>" <?php echo $hide_slider ? "data-masonry-options='{  \"itemSelector\": \".item\" }'" : 'data-equalizer' ?>>
