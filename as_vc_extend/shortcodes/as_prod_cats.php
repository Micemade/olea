...
/*	if ( $enter_anim != 'none' && !wp_style_is( 'animate', 'enqueued' )) {wp_register_style( 'animate', get_template_directory_uri() . '/css/animate.css');wp_enqueue_style( 'animate' );} */
	...
	echo '<div id="prod-cats-'.$block_id.'">';
		...
		if( !empty($term_Objects) ) {
			...
			foreach ( $term_Objects as $term_obj ) {
				...
				if( $prod_cat_menu == 'images' ) { // if images should be displayed:
					...
					if ( $image ) {
						...
						echo '<img src="' . esc_url( bfi_thumb( $image[0], array( 'width' => $img_width, 'height' => $img_height) ) ) . '" alt="" />';
						echo '</a>';
						echo '</li>';
						...
						echo '<img src="' . esc_url( bfi_thumb( AS_PLACEHOLDER_IMAGE, array( 'width' => $img_width, 'height' => $img_height) ) ). '" alt="" />';
						echo '</a>';
						echo '</li>';
					}
			...
			echo '<li>'. __("No product category exists","olea") . '</li>';
	...
	<?php
		...
		echo '<h5 class="no-woo-notice">' . __('PRODUCT CATEGORIES BLOCK DISABLED.<br> Sorry, it seems like WooCommerce is not active. Please install and activate last version of WooCommerce.','olea') . '</h5>';
			return;
	} // if $olea_woo_is_active
