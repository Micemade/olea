...
if( $api_key !== '' ) {
	...
	<div id="map-<?php echo esc_attr($block_id); ?>-holder" class="inner-wrapper<?php echo ($enter_anim != 'none') ? ' to-anim' :''; echo $css_classes ? ' '. esc_attr($css_classes) : ''; ?>" >
	...
	if( $prepAddr && !$latitude && !$longitude ) {
		...
		if( empty( $output->results ) || ($output->status != 'OK') ) {
			echo '<h3 style="text-align:center">' . esc_html__("Google maps error","olea") .' :</h3>';
			echo '<p style="text-align:center">' . esc_html__("Please check your address inputs - there's a probable error in data, or use manual longitude and latitude inputs.","olea") .'</p>';
			return;
		} 
	...
	?>
		<div style="text-align: center; padding: 30px; margin: 30px; border: 1px solid #999;">
			<h5><?php echo esc_html__('Google Maps Alert','olea') ?></h5>
			<br>
			<?php 
			$gmap_api_url = "https://developers.google.com/maps/documentation/javascript/get-api-key";
			echo esc_html__("Missing API key for Google Maps.",'olea');
			echo '<a href="'. esc_url( $gmap_api_url ) .'" target="_blank">' . esc_html__("Get your API key here ",'olea') .'</a>'; ?>
		</div>
