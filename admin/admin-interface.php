...
<div class="wrap" id="of_container">
  ...
  <form id="of_form" method="post" action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" enctype="multipart/form-data" >
	...
	<button id ="of_save" type="button" class="button-primary"><?php _e('Save All Changes','olea');?></button>
	</div><!--.info_bar--> 	
    ...
    <div id="main">
      <div id="of-nav">
        <ul>
		  ...
		  //echo html_entity_decode( $options_machine->Menu , ENT_HTML5 , 'UTF-8' ); ?>
        </ul>
      </div>
      <div id="content"> 
	  <?php
	  echo wp_kses_decode_entities($options_machine->Inputs ); // Settings
	  //echo html_entity_decode($options_machine->Inputs, ENT_HTML5 , 'UTF-8' ); // Settings  
	  ?> </div>
      <div class="clear">
	  </div>
    </div>
	...
	<button id ="of_save" type="submit" class="button-primary"><?php esc_html_e('Save All Changes','olea');?></button>
	<button id ="of_reset" type="submit" class="button submit-button reset-button" ><?php esc_html_e('Options Reset','olea');?></button>
	</div><!--.save_bar--> 
 
  </form>

 
<?php  if (!empty($update_message)) echo html_entity_decode( $update_message,  ENT_HTML5 , 'UTF-8' ); ?>
...
function of_admin_head() { 
	...
	*/
	( function( $ ) {
	window.GoogleFontSelect = function ( slctr, mainID ) {
		...
		if( _selected ){ //if var exists and isset
			...
			if ( _selected !== 'none' && _selected !== 'Select a font' ) {
				...
				//ADD LINK TO GOOGLE FONT FAMILY
	...
	$('#expand_options').click(function(){
		if(flip == 0){
			...
			$('#of_container #content').css('width','100%');
			...
			$('#of_container #content').css('width','75%');
	...
	// Reset Message Popup
	var reset = "<?php if(isset($_REQUEST['reset'])) echo html_entity_decode( $_REQUEST['reset'] , ENT_HTML5 , 'UTF-8' ); ?>";
	...
	$('#of_backup_button').live('click', function(){
		...
		var answer = confirm("<?php _e('Click OK to backup your current saved options.','olea');?>")
	...
	$('#of_restore_button').live('click', function(){
		...
		var answer = confirm("<?php _e('Warning: All of your current options will be replaced with the data from your last backup! Proceed?','olea');?>")
	...
	$('#of_reset').click(function() {
		var answer = confirm("<?php _e('Click OK to reset. All settings will be lost!','olea');?>")
	...
	*/
	...
	jQuery( '.google_font_select' ).each(function(){ 
		...
		GoogleFontSelect( jQuery(this), mainID );
	});
...
function of_ajax_callback() {
	...
	elseif($save_type == 'import_options'){
		...
		$as_of = unserialize( urldecode( $_POST['data'] ) );
...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		//$output .= '<input class="position" type="hidden" name="'.$value['id'].'['.$group.']['.$key.']" value="'.$list[0].$list[1].'" id="'.$list[0].'">';
			...
			case 'backup':
				...
				$output .= '<p><strong>'. __('Last Backup : ','olea').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
			...
			case 'transfer':
				...
				$output .= '<textarea id="export_data" rows="8">'. urlencode( serialize( $as_of ) ) .'</textarea>'."\n";
	...
	public static function optionsframework_uploader_function($id,$std,$mod){
		...
		$uploader .= '<div class="upload_button_div"><span class="button image_upload_button" id="'.$id.'">'._('Upload', 'olea').'</span>';
	...
	public static function optionsframework_icons_function($id,$std,$oldorder,$order){
		...
		$icons_holder .= '<select id="'.$id.'_'.$order .'-icon" name="'. $id .'['.$order.'][icon]" class="select-icons">';
			...
			//$icons_holder .= '<option id="'.$key.'-empty" value="'.$key.'-empty" '.selected( $val['icon'], $key, false ).'></option>';
