...
class Micemade_GitHub_Theme_Updater {
	...
	protected function __construct() {
		add_filter( 'extra_theme_headers', array($this, 'mm_gtu_add_headers') );
		$this->mm_gtu_get_github_themes();
		if( !empty($_GET['action'] ) && ( $_GET['action'] == 'do-core-reinstall' || $_GET['action'] == 'do-core-upgrade') ); else {
			add_filter( 'site_transient_update_themes', array( $this, 'mm_gtu_transient_update_themes_filter') );
		}
		add_filter( 'upgrader_source_selection', array( $this, 'mm_gtu_upgrader_source_selection_filter' ), 10, 3 );
		add_action( 'http_request_args', array( $this, 'mm_gtu_no_ssl_http_request_args' ), 10, 2 );
	 ...
	 * @return array
	 */
	public function mm_gtu_add_headers( $extra_headers ) {
	 ...
	 * @since 1.0
	 */
	private function mm_gtu_get_github_themes() {
	 ...
	 * @return array|object
	 */
	public function mm_gtu_transient_update_themes_filter( $data ){
	 ...
	 * @return string 
	 */
	public function mm_gtu_upgrader_source_selection_filter( $source, $remote_source=NULL, $upgrader=NULL ) {
		if( isset( $source ) )
	...
	public function mm_gtu_no_ssl_http_request_args( $args, $url ) {
...
Micemade_GitHub_Theme_Updater::instance();