...
abstract class AS_Field {
	...
	public function __construct( $name, $title, array $values, $args = array() ) {
		...
		if ( ! empty( $this->args['std'] ) && empty( $this->args['default'] ) ) {
			...
			_deprecated_argument( 'AS_Field', "'std' is deprecated, use 'default instead'", '0.9' );
		}
	...
	public function get_the_id_attr( $append = null ) {
		...
		if ( isset( $this->parent ) ) {
			$parent_id = preg_replace( '/as\-field\-(\d|x)+/', 'as-group-$1', $this->parent->get_the_id_attr() );
			$id = $parent_id . '[' . $id . ']';
		}

		$id .= '-as-field-' . $this->field_index;
	...
	public function get_the_name_attr( $append = null ) {
		...
		if ( isset( $this->parent ) ) {
			$parent_name = preg_replace( '/as\-field\-(\d|x)+/', 'as-group-$1', $this->parent->get_the_name_attr() );
			$name = $parent_name . '[' . $name . ']';
		}

		$name .= "[as-field-$this->field_index]";
	...
	public function description() {
		...
		if ( $this->description ) { ?>
			...
			<div class="as_metabox_description"><?php echo wp_kses_post( $this->description ); ?></div>
	...
	public function display() {
		...
		foreach ( $values as $key => $value ) {
			...
			<?php if ( $this->args['repeatable'] ) : ?>
				<button class="as-delete-field" title="Remove field"><span class="as-delete-field-icon">&times;</span></button>
			<?php endif; ?>
		...
		if ( $this->args['repeatable'] ) {
			...
			<?php if ( $this->args['repeatable'] ) : ?>
				<button class="as-delete-field" title="Remove field"><span class="as-delete-field-icon">&times;</span> Remove Group</button>
			<?php endif; ?>
			...
			<button class="button repeat-field"><?php esc_html_e( 'Add New', 'olea' ); ?></button>
 ...
 * @extends AS_Field
 */
class AS_Text_Field extends AS_Field {
...
class AS_Text_Small_Field extends AS_Text_Field {

	public function html() { 

		$this->args['class'] .= ' as_text_small';	
 ...
 */
class AS_File_Field extends AS_Field {

	function enqueue_scripts() {

		parent::enqueue_scripts();
		wp_enqueue_media();
		wp_enqueue_script( 'as-file-upload', trailingslashit( AS_URL ) . 'js/file-upload.js', array( 'jquery', 'as-scripts' ) );
	...
	public function html() { 
		...
		<div class="as-file-wrap" <?php echo 'data-type="' . esc_attr( $data_type ) . '"'; ?>>

			<div class="as-file-wrap-placeholder"></div>

			<button class="button as-file-upload <?php echo $this->get_value() ? 'hidden' : '' ?>">
				<?php esc_html_e( 'Add File', 'olea' ); ?>
			</button>

			<div class="as-file-holder type-file <?php echo $this->get_value() ? '' : 'hidden'; ?>">

				<?php if ( $this->get_value() ) : ?>

					<?php if ( isset( $icon_img ) ) echo wp_kses_post($icon_img); ?>

					<div class="as-file-name">
			...
			<button class="as-remove-file button <?php echo $this->get_value() ? '' : 'hidden'; ?>">
				<?php esc_html_e( 'Remove', 'olea' ); ?>
			</button>

			<input type="hidden" class="as-file-upload-input" <?php $this->name_attr(); ?> value="<?php echo esc_attr( $this->value ); ?>" />
...
class AS_Image_Field extends AS_File_Field {
	...
	public function html() {
		...
		<div class="as-file-wrap" style="<?php echo esc_attr( $styles ); ?>" data-type="<?php echo esc_attr( $data_type ); ?>">

			<div class="as-file-wrap-placeholder" style="<?php echo esc_attr( $placeholder_styles ); ?>">
			...
			<button class="button as-file-upload <?php echo esc_attr( $this->get_value() ) ? 'hidden' : '' ?>" data-nonce="<?php echo wp_create_nonce( 'as-file-upload-nonce' ); ?>">
				<?php esc_html_e( 'Add Image', 'olea' ); ?>
			</button>

			<div class="as-file-holder type-img <?php echo $this->get_value() ? '' : 'hidden'; ?>" data-crop="<?php echo (bool) $size['crop']; ?>">
	...
	static function request_image_ajax_callback() {
		...
		if ( ! ( isset( $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], 'as-file-upload-nonce' ) ) )
			return;
...
add_action( 'wp_ajax_as_request_image', array( 'AS_Image_Field', 'request_image_ajax_callback' ) );

/**
 * Standard text meta box for a URL.
 *
 */
class AS_URL_Field extends AS_Field {

	public function html() { ?>

		<input type="text" <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'as_text_url code' ); ?> <?php $this->name_attr(); ?> value="<?php echo esc_attr( esc_url( $this->value ) ); ?>" />
 ...
 *
 */
class AS_Date_Field extends AS_Field {

	public function enqueue_scripts() {

		parent::enqueue_scripts();

		wp_enqueue_style( 'as-jquery-ui', trailingslashit( AS_URL ) . 'css/vendor/jquery-ui/jquery-ui.css', '1.10.3' );

		wp_enqueue_script( 'as-datetime', trailingslashit( AS_URL ) . 'js/field.datetime.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-datepicker', 'as-scripts' ) );
	}

	public function html() { ?>

		<input <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'as_text_small as_datepicker' ); ?> type="text" <?php $this->name_attr(); ?> value="<?php echo esc_attr( $this->value ); ?>" />

	<?php }
}

class AS_Time_Field extends AS_Field {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'as-timepicker', trailingslashit( AS_URL ) . 'js/jquery.timePicker.min.js', array( 'jquery', 'as-scripts' ) );
		wp_enqueue_script( 'as-datetime', trailingslashit( AS_URL ) . 'js/field.datetime.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-datepicker', 'as-scripts' ) );
	}

	public function html() { ?>

		<input <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'as_text_small as_timepicker' ); ?> type="text" <?php $this->name_attr(); ?> value="<?php echo esc_attr( $this->value ); ?>"/>
 ...
 *
 */
class AS_Date_Timestamp_Field extends AS_Field {
	...
	public function html() { ?>
		...
		<input <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'as_text_small as_datepicker' ); ?> type="text" <?php $this->name_attr(); ?>  value="<?php echo $this->value ? esc_attr( date( 'm\/d\/Y', $this->value ) ) : '' ?>" />
 ...
 *
 */
class AS_Datetime_Timestamp_Field extends AS_Field {
	
	public function enqueue_scripts() {

		parent::enqueue_scripts();

		wp_enqueue_style( 'as-jquery-ui', trailingslashit( AS_URL ) . 'css/jquery-ui.css', '1.10.3' );
	...
	public function html() { ?>		
		...
		<input <?php $this->id_attr('date'); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'as_text_small as_datepicker' ); ?> type="text" <?php $this->name_attr( '[date]' ); ?>  value="<?php echo $this->value ? esc_attr( date( 'm\/d\/Y', $this->value ) ) : '' ?>" />
		<input <?php $this->id_attr('time'); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'as_text_small as_timepicker' ); ?> type="text" <?php $this->name_attr( '[time]' ); ?> value="<?php echo $this->value ? esc_attr( date( 'h:i A', $this->value ) ) : '' ?>" />
 ...
 */
class AS_Textarea_Field extends AS_Field {
 ...
 *  - int "rows" - number of rows in the <textarea>
 */
class AS_Textarea_Field_Code extends AS_Textarea_Field {
 ...
 *
 */
class AS_Color_Picker extends AS_Field {

	public function enqueue_scripts() {

		parent::enqueue_scripts();

		wp_enqueue_script( 'as-colorpicker', trailingslashit( AS_URL ) . 'js/field.colorpicker.js', array( 'jquery', 'wp-color-picker', 'as-scripts' ) );
		wp_enqueue_style( 'wp-color-picker' );
	}

	public function html() { ?>

		<input <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'as_colorpicker as_text_small' ); ?> type="text" <?php $this->name_attr(); ?> value="<?php echo esc_attr( $this->get_value() ); ?>" />
 ...
 */
class AS_Radio_Field extends AS_Field {
 ...
 *
 */
class AS_Checkbox extends AS_Field {
 ...
 *
 */
class AS_Title extends AS_Field {
 ...
 *
 */
class AS_wysiwyg extends AS_Field {

	function enqueue_scripts() {

		parent::enqueue_scripts();

		wp_enqueue_script( 'as-wysiwyg', trailingslashit( AS_URL ) . 'js/field-wysiwyg.js', array( 'jquery', 'as-scripts' ) );
	}
	...
	public function html() { 
		...
		printf( '<div class="as-wysiwyg" data-id="%s" data-name="%s" data-field-id="%s">', $id, $name, $field_id );
	
		if ( $this->is_placeholder() ) 	{

			// For placeholder, output the markup for the editor in a JS var.
			ob_start();
			$this->args['options']['textarea_name'] = 'as-placeholder-name-' . $field_id;
			wp_editor( '', 'as-placeholder-id-' . $field_id, $this->args['options'] );
			$editor = ob_get_clean();
			...
			<script>
				if ( 'undefined' === typeof( as_wysiwyg_editors ) ) 
					var as_wysiwyg_editors = {};
				as_wysiwyg_editors.<?php echo esc_js($field_id); ?> = '<?php echo esc_js($editor); ?>';
			</script>
 ...
 */
class AS_Select extends AS_Field {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'select2', get_template_directory_uri() . '/js/select2/select2.js', array( 'jquery' ) );
		wp_enqueue_script( 'field-select', trailingslashit( AS_URL ) . 'js/field.select.js', array( 'jquery', 'select2', 'as-scripts' ) );
	}
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( 'select2', get_template_directory_uri() . '/js/select2/select2.css' );
	}
	...
	public function output_field() {
		...
		>
			...
			<?php if ( ! empty( $this->args['allow_none'] ) ) : ?>
				<option value=""><?php echo esc_html_x( 'None', 'select field', 'olea' ) ?></option>
			<?php endif; ?>
	...
	public function output_script() {
		...
		<script type="text/javascript">
			...
			(function($) {
				...
				options.placeholder = <?php echo json_encode( __( 'Type to search', 'olea' ) ) ?>;
				options.allowClear  = true;

				if ( 'undefined' === typeof( window.as_select_fields ) )
					window.as_select_fields = {};
				
				var id = <?php echo json_encode( $this->get_js_id() ); ?>;
				window.as_select_fields[id] = options;
...
class AS_Taxonomy extends AS_Select {
 ...
 *     'multiple'     => bool whether multiple can be selected
 */
class AS_Post_Select extends AS_Select {
	...
	public function output_script() {
		...
		<script type="text/javascript">
			...
			(function($) {
				...
				var id = <?php echo json_encode( $this->get_js_id() ); ?>;
				var options = window.as_select_fields[id];
				...
				<?php if ( $this->args['ajax_url'] ) : ?>
					...
					var ajaxData = {
						action  : 'as_post_select',
						post_id : '<?php echo intval( get_the_id() ); ?>', // Used for user capabilty check.
						nonce   : <?php echo json_encode( wp_create_nonce( 'as_select_field' ) ); ?>,
...
function as_ajax_post_select() {
	...
	if ( ! $nonce || ! wp_verify_nonce( $nonce, 'as_select_field' ) || ! current_user_can( 'edit_post', $post_id ) ) {
...
add_action( 'wp_ajax_as_post_select', 'as_ajax_post_select' );
 ...
 */
class AS_Group_Field extends AS_Field {
	...
	function __construct() {
		...
		if ( ! empty( $this->args['fields'] ) ) {
			foreach ( $this->args['fields'] as $f ) {
				...
				$class = _as_field_class_for_type( $f['type'] );
				$f['show_label'] = true;
	...
	public function display() {
		...
		if ( $this->args['repeatable'] ) {
			...
			$this->value = ''; ?>
				...
				<button class="button repeat-field"><?php esc_html_e( 'Add New Group', 'olea' ); ?></button>
	...
	public function html() {
		...
		<?php AS_Meta_Box::layout_fields( $fields ); ?>
	...
	public function add_field( AS_Field $field ) {
		$field->parent = $this;
