...
class AS_Meta_Box {
	...
	function __construct( $meta_box ) {
		...
		add_action( 'as_init_fields', array( &$this, 'init_fields' ) );
		...
		add_action( 'as_save_fields', array( &$this, 'save' ) );

		add_action( 'admin_enqueue_scripts', array( &$this, 'enqueue_scripts' ) );
		add_action( 'admin_enqueue_scripts', array( &$this, 'enqueue_styles' ) );

		add_filter( 'as_show_on', array( &$this, 'add_for_id' ), 10, 2 );
		add_filter( 'as_show_on', array( &$this, 'for_page_template' ), 10, 2 );
	...
	public function init_fields( $post_id = 0 ) {
		...
		foreach ( $this->_meta_box['fields'] as $key => $field ) {
			...
			$class = _as_field_class_for_type( $field['type'] );
	...
	function enqueue_styles() {
		...
		if ( version_compare( get_bloginfo( 'version' ), '3.8', '>=' ) )
			wp_enqueue_style( 'as-styles', trailingslashit( AS_URL ) . "css/dist/as$suffix.css" );
		else
			wp_enqueue_style( 'as-styles', trailingslashit( AS_URL ) . 'css/legacy.css' );
	...
	function add() {
		...
		foreach ( (array) $this->_meta_box['pages'] as $page )
			if ( apply_filters( 'as_show_on', true, $this->_meta_box ) )
	...
	/**
	 * Show On Filters
	 * Use the 'as_show_on' filter to further refine the conditions under which a metabox is displayed.
	...
	function for_page_template( $display, $meta_box ) {
	...
	static function layout_fields( array $fields ) { ?>
		...
		<div class="as_metabox">
			...
			foreach ( $fields as $field ) :
				...
				<div class="as-cell-<?php echo intval( $field->args['cols'] ); ?>">
					
						<div <?php echo implode( ' ', $attrs ); ?>>
							<?php $field->display(); ?>
						</div>

						<input type="hidden" name="_as_present_<?php echo esc_attr( $field->id ); ?>" value="1" />
	...
	function strip_repeatable( $values ) {
		...
		foreach ( $values as $key => $value ) {
			...
			if ( false !== strpos( $key, 'as-group-x' ) || false !==  strpos( $key, 'as-field-x' ) )
				unset( $values[$key] ); 
	...
	function save( $post_id = 0 )  {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( ! isset( $_POST['_as_present_' . $field['id'] ] ) )
				continue;
			...
			if ( ! $class = _as_field_class_for_type( $field['type'] ) ) {
				do_action('as_save_' . $field['type'], $field, $value);
			}
