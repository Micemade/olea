...
function as_post_formats_media( $post_id, $block_id=null, $img_format=null, $img_width=null, $img_height=null ) {
	...
	if( $custom_head_image ) {
		$img_url			= as_get_full_img_url( $custom_head_image );
		$image_output		= wp_get_attachment_image( $custom_head_image, $img_format );
		$pP_rel				= '';
		$img_urls_gallery	= ''; // avoid duplicate gallery image urls
		$quote_html			= '';
	}
	elseif( $post_format == 'video' ) { // <---------- GALLERY POST VIDEO
		...
		if ( $featured_or_thumb == 'host_thumb' ) { // if thumbs from video hosts
			...
			$image_output = '<div class="entry-image"><img src="'. esc_url( bfi_thumb( $img , array( 'width' => $img_width, 'height' =>  $img_height) ) ) .'" alt="'. the_title_attribute (array('echo' => 0)).'" /></div>';
