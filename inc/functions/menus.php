<?php
/**
 * Custom menu fields - template functions
 *
...
class AS_Nav_Custom_Fields {
	...
	static function setup() {
		...
		self::$options['fields'] = array(
			...
			'mega'			=>array(
				'name'				=> 'megamenu',
				'label'				=> __('Mega menu holder', 'olea'),
				'container_class'	=> 'mega-menu',
				'input_type' 		=> 'checkbox'
			),
			'clear'			=> array(
				'name'				=> 'clear',
				'label'				=> __('Clear for row ?', 'olea'),
				'container_class'	=> 'mega-menu-clear',
				'input_type'		=> 'checkbox'
			),
			'invisible'	=> array(
				'name'				=> 'stretcher',
				'label'				=> __('Invisible item (stretcher)', 'olea'),
				'container_class'	=> 'mega-menu-stretcher',
				'input_type'		=> 'checkbox'
			),
			'title'	=> array(
				'name'				=> 'section_title',
				'label'				=> __('Item as section title', 'olea'),
				'container_class'	=> 'mega-menu-title',
				'input_type'		=> 'checkbox'
			),
			'image'			=> array(
				'name'				=> 'image',
				'label'				=> __('Custom image', 'olea'),
				'container_class'	=> 'mega-menu-image',
				'input_type'		=> 'image'
			),
			'post_thumb'	=> array(
				'name'				=> 'post_thumb',
				'label'				=> __('Post thumb and excerpt ?', 'olea'),
				'container_class'	=> 'mega-menu-postthumb',
				'input_type'		=> 'checkbox'
			),
			'icon'	=> array(
				'name'				=> 'icon',
				'label'				=> __('Select icon for item', 'olea'),
	...
	static function _add_fields($new_fields, $item_output, $item, $depth, $args, $current_object_id) {
		...
		foreach( $schema as $field ) {
			...
			if( $field['input_type'] == 'image') {
				...
				$options = !empty( $icons_arr )  ? $icons_arr : array();
...
{	
	...
	function start_lvl( &$output, $depth = 0, $args=array() ) {
		...
		$output .= '<ul class="sub-menu'. ( $is_mega ? ' as-megamenu' : '' ). '"'.$backimg.'>';
	}
	...
	function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
		...
		if( is_object($args) ) {
			...
			//$class_names = rtrim($class_names, '"').' large-3 column "';
			//		
			...
			if( !empty( $has_children ) ){
				...
				if( $orientation == 'vertical' ) {
					$arrow = ' <span class="icon-chevron-right menu-icon-arrow"></span>';
				}else{
					$arrow = ' <span class="icon-chevron-down menu-icon-arrow"></span>';
				}
			...
			if ( $depth >= 1 && $post_thumb ) {
				...
				$output .= '<a href="'.esc_attr( $item->url ).'" class="button">'. __('Read more','olea') .'</a>';
...
function olea_wp_nav_menu( $text ) {
$replace = array(
...
add_filter('wp_nav_menu', 'olea_wp_nav_menu');
//
?>