...
function as_archive_content_func() {
	...
	if( !post_password_required() ) {
		...
		if( strpos( $post->post_content, '<!--more-->' ) ) {
			$readmore = '<a class="button readmore" href="'. get_permalink( get_the_ID() ) . '">' . esc_html(__('Read More', 'olea')) . ' &rarr;</a>';
		...
		echo '<p>' . esc_html__("This post is password protected","olea") . '</p>';
	}
...
function new_excerpt_more( $more ) {
	return ' <a class="button readmore" href="'. get_permalink( get_the_ID() ) . '">' . esc_html(__('Read More', 'olea')) . ' &rarr;</a>';
}
...
function as_edit_post_link() {
	...
	$edit_post 		= $post_edit_link ? ' <a href="'. $post_edit_link .'" class="post-edit-link">'. __('Edit this','olea') .'</a>' : '';
	return $edit_post;
}
/**
...
function as_custom_excerpt_func( $chars = 150, $has_readmore = true) {
	...
	$readmore	= '<br /><a href="'.get_permalink().'" class="tiny button">'. esc_html('Read more','olea') .'</a>';
	$text		= $text . " ";
...
function as_prev_next_post() {
	...
	$output = '<nav class="nav-single">';
		...
		$prevTitle_s = (strlen($prevTitle) > 43) ? substr($prevTitle,0,40).'...' : $prevTitle;
		$prevPrefix = __('Previous : ','olea');
		

		$nextPost	= get_next_post();
		$nextURL	= $nextPost ? get_permalink($nextPost->ID) : '';
		$nextTitle	= $nextPost ? $nextPost->post_title : '';
		$nextTitle_s = (strlen($nextTitle) > 43) ? substr($nextTitle,0,40).'...' : $nextTitle;
		$nextPrefix = __('Next : ','olea');
		...
		$output .= '<span class="nav-previous">';
			...
			$output .= '<span class="title">'. esc_html($prevPrefix) .'<br>'. esc_html($prevTitle_s)  .'</span></span>';
			$output .= '</a>';
			$output .= '</span>';
		};
		...
		$output .= '<span class="nav-next">';
			...
			$output .= '<span class="title">'. esc_html($nextPrefix) .'<br>'. esc_html($nextTitle_s)  .'</span></span>';
			$output .= '</a>';
			$output .= '</span>';
		};
...
function as_inner_custom_box( $post ) {
	...
	*/
	...
	echo '<label for="as_featured_field">'. _e( "Make this item featured", 'olea' ) .'</label>';
