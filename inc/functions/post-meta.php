...
if ( ! function_exists( 'as_entry_author' ) ) {
	function as_entry_author() {
		...
		// author:
		$by  	=  __('author: ','olea');
		$output = '<a class="url fn n author" href="'.$author_link.'" title="'. esc_attr(__('View all post by ','olea') .$author ).'" rel="author">'.$by.$author.'</a>';
				
		$edit_post = as_edit_post_link(); // in misc_post_functions.php
		
		echo $author ? wp_kses_post($output.$edit_post) : $edit_post;
	}
}
...
function as_entryMeta_dateUser() {
	...
	$full_date = esc_attr( get_the_date( 'c' ) );
	$date_formatted = esc_html( get_the_date( apply_filters( "olea_options", "post_date_format", "d M Y" ) ) );
	...
	$output .= '<span class="hover-box"><a class="url fn n author vcard" href="'.$author_link.'" title="'. esc_attr(__('View all post by ','olea') .$author ).'" rel="author">'.$author.'</a><span class="arrow-down"></span></span>';
	$output .= '</span>';
