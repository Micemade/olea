...
function as_admin_bar_render() {
    ...
    $wp_admin_bar->add_menu( array(
        ...
        'title'	=> __('THEME DOCUMENTATION','olea'),
...
function custom_header_style () {
	...
	$custom_style .=  '}';
	//header fonts
	$custom_style .=  $h_fonts_color ? '#site-menu .topbar-info, #site-menu:not(.sticked) .olea-head-cart, #site-menu .breadcrumbs span, #site-menu .topbar-toggle { color: '. $h_fonts_color .' }' : '';
	// header links
	...
	$custom_style .=  '#site-menu.horizontal .topbar, .horizontal ul.navigation > li > a:before,  #site-menu.horizontal .border-bottom, #site-menu.horizontal .border-top, .searchform-menu input[type="search"], .horizontal #main-nav-wrapper ul.navigation > li > a { border-color:'.$border_c.' !important; } ';
	...
	echo html_entity_decode( $custom_style , ENT_QUOTES, 'UTF-8' );
}
...
function wpa82718_scripts() {
    ...
    $colorpicker_l10n = array(
        'clear' => __( 'Clear','olea' ),
        'defaultString' => __( 'Default','olea' ),
        'pick' => __( 'Select Color','olea' )
    );
...
if ( !function_exists('blockusers_init') ) {
	...
	function blockusers_init() {
		...
		if( apply_filters( "olea_options", "blockusers", 1 ) ) {
 ...
 */
function olea_remove_deprecated_templates() {
	
	$files_to_remove = array( 
		// since Olea 1.3.0 , these files are redundant:
...
add_action('admin_init', 'olea_remove_deprecated_templates');
/**
 *	REMOVE LOOKBOOK PAGE META EVERYWHERE EXPECT IN LOOKBOOK PAGE TEMPLATE:
 *
 */
