...
if ( ! function_exists( 'as_video_thumbs' ) ) {
	...
	function as_video_thumbs() {
		...
		if( $host == 'youtube' ) {
			...
			$json_url	= wp_remote_get('http://www.screenr.com/api/oembed.json?url=http://www.screenr.com/'.$video_id);
			$scr_array	= json_decode($json_url, true);
			$img_url	= $scr_array['thumbnail_url'];
		
		}elseif ( $host == 'vimeo' ){ 
			
			$video_data		= wp_remote_get("http://vimeo.com/api/v2/video/$video_id.php");
			$unserialized	= unserialize( $video_data['body'] );
			$img_url		= $unserialized[0]['thumbnail_large'];
			...
			$blip_xml = wp_remote_get( 'http://blip.tv/rss/'. $video_id );
