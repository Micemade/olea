...
function as_gallery_output( $postid, $images_ids, $slider_thumbs = 'slider', $thumb_columns = 3, $thumb_size = 'thumbnail' , $number_posts = -1 ) {
	...
	if ( $gallery_images ) {
		...
		foreach( $gallery_images as $image ) :
			...
			$output .= '<div class="back-buttons"><a data-rel="prettyPhoto[gallery_'.$pid.']" rel=""  href="'. $img_url .'" title="'. the_title_attribute (array('echo' => 0)) . ' ">';
...
function as_post_gallery( $output, $attr ) {
    ...
    foreach ( $attachments as $id => $attachment ) {
		...
		$output .= '<div class="back-buttons"><a data-rel="prettyPhoto[gallery_'.$post->ID .'-'.$instance.']" rel="" href="'. $img_url .'" title="'. the_title_attribute (array('echo' => 0)) . ' " class="zoom">';
		$output .= '<div class="icon icon-zoom-in" aria-hidden="true"></div></a>';
		$output .= '</div>'; // .back-buttons
		...
		if ( $captiontag && ( $caption || $desc ) ) {
			
			$output .= "<{$captiontag} class='gallery-caption'>";
			if( $caption ) {
				$output .=  '<strong>' .wptexturize($attachment->post_excerpt). '</strong>';
			}
			if( $desc ) {
