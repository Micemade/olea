...
$smooth_wheelscroll		= apply_filters( "olea_options", "smooth_wheelscroll", 1 );
$use_nice_scroll_menus	= apply_filters( "olea_options", "use_nice_scroll_menus", 1 );
$predefined_headers		= apply_filters( "olea_options", "predefined_headers", "01" );
$use_preloader			= apply_filters( "olea_options", "use_preloader", 0 ) || $predefined_headers == 'simple' ? true : false;
$demo_mode				= apply_filters( "olea_options", "demo_mode", 0 );
...
$c(document).ready(function() {	
	...
	$c(document).on("click", "a.ajax-products", function(e) {
		...
		$c.ajax({
			type: "POST",
			url: ajaxurl,
			data: {"action": "load-filter", block_id: block_id, termID: t_ID, tax: taxonomy, post_type: ptype, total_items: totitems, filters: data_filters,  img_format: img,  shop_quick: shop_quick,  shop_buy_action: shop_buy_action, shop_wishlist: shop_wishlist, enter_anim: enter_anim, no_slider: no_slider, zoom_button: zoom_button, link_button: link_button, hide_prod_info: hide_prod_info },
			success: function(response) {
				...
				if( cat_content.hasClass("contentslides") ) {
					...
					cat_content.masonry("reloadItems");   
	...
	$c(document).on("click", "#options>div a", function(e) {
		...
		$c.ajax({
			...
			success: function(response) {
				...
				if( skin_to_load == "montserrat" ) {
					fontHead = "Montserrat";
					fontBody = "Montserrat";
				}else if ( skin_to_load == "narrow_red_rustic" ){
					fontHead = ""; //fontHead = "Amatic+SC";
					fontBody = ""; //fontBody = "Lato";
				}else if ( skin_to_load == "narrow_titles" ){
					fontHead = "Fjalla+One";
					fontBody = "Raleway";
				}else if ( skin_to_load == "open_sans_light" ){
					fontHead = "Open+Sans";
					fontBody = "Cabin";
				}else if ( skin_to_load == "playfair" ){
					fontHead = "Playfair+Display";
					fontBody = "Lato";
				}else if ( skin_to_load == "rose_sorts_mill_goudy" ){
					fontHead = "Sorts+Mill+Goudy";
					fontBody = "Dosis";
				}
...
echo html_entity_decode( $theme_js , ENT_QUOTES, 'UTF-8' );
?>